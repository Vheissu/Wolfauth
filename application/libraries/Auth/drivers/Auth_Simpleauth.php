...
class Auth_simpleauth extends CI_Driver {
    ...
    public function __construct()
    {
        ...
        // Alias to the Simpleauth model
        $this->auth_model = $this->CI->simpleauth_model;
        ...
        $this->capabilities = $this->auth_model->get_capabilities($user->role_title);
    }
    ...
    public function is_role($slug)
    {
        ...
        return ($slug === $this->role['role']) ? TRUE : FALSE;
    }
    ...
    public function get_user($as_object = FALSE)
    {
        ...
        {
            // Get the user by ID
            $user = $this->auth_model->get_user_by_id($this->user_id(), 'user_id');
    ...
    public function user_can($capability, $user_id = 0)
    {
        ...
        if ($user)
        {
            $capabilities = $this->auth_model->get_capabilities($user->row("role_title"));

            if ($capabilities)
            {
                foreach ($capabilities AS $capability_temp)
                {
                    if ($capability_temp->capability == $capability)
                    {
                        return TRUE;
                    }
                }
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $user = $this->auth_model->get_user($identity);
        ...
        if ($user)
        {
            ...
            if ($this->hash($password) == $user->row('password'))
            {
                ...
                $attempts = $this->auth_model->get_login_attempts($username);

                // We are under or on the border line for logiin attempts
                if ($attempts <= $this->_config['login.max_attempts'])
                {
                ...
                {
                    $this->set_error($this->CI->lang->line("error_max_login_attempts"));
                }
            }
            else
            {
                $this->set_error($this->CI->lang->line("error_login_details"));
            }
        }
    ...
    public function logout($redirect = '')
    {
        ...
        $this->auth_model->update_user($user_data);
    ...
    private function _set_remember_me($user_id)
    {
        ...
        set_cookie($cookie);
        $this->auth_model->update_user(array('id' => $user_id, 'remember_me' => $remember_me));
    }
    ...
    private function _check_remember_me()
    {
        ...
        if($cookie_data = get_cookie('rememberme'))
        {
            ...
            if ($data = $this->auth_model->get_user_by_id($user_id))
            {
                // Set session values
                $this->CI->session->set_userdata(array(
                    'user_id'   => $user_id,
